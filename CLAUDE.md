...
### Production
PM2 entry at `ecosystem.config.cjs` reads `~/.claude/settings.json` `env` block at start time and forwards `ANTHROPIC_BASE_URL` and the per-tier `ANTHROPIC_DEFAULT_*_MODEL` vars into the process env. Auth is OAuth-only via `~/.anthropic/auth.json`, matching system Claude Code auth. Changing settings or refreshing OAuth credentials requires a PM2 restart. CORS allows configured `ALLOWED_ORIGINS` plus localhost and RFC1918 ranges unconditionally.
