...
function renderActivityStatus(session) {
	...
	if (!session.activityState) {
		if (session.isStreaming) {
			...
			indicatorEl.className = "activity-indicator thinking";
			labelEl.textContent = "CLI session live";
...
function markPlanConfirmationSubmitted(element, status) {
	...
	if (actions)
		setElementHtml(
			actions,
			`<span class="plan-status plan-status-${status}">${status === "approved" ? "Plan approved" : "Plan rejected — revising..."}</span>`,
		);
