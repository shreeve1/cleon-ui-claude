...
import { EventDelivery, eventDelivery } from "../../server/event-delivery.js";
...
describe("EventDelivery", () => {
	...
	it("drops oldest transcript events and emits replay-truncated marker", () => {
		...
		delivery.recordSessionEvent("session-2", {
			type: "claude-message",
			sessionId: "session-2",
			data: { content: "one" },
		});
	...
	it("replays only latest Activity State, not activity history", () => {
		...
		delivery.recordSessionEvent("session-3", {
			type: "agent-activity",
			sessionId: "session-3",
			state: "thinking",
			label: "Thinking...",
	...
	it("replays Task Events and appends Task State Snapshot", () => {
		...
		delivery.recordSessionEvent("session-4", {
			type: "task-started",
			sessionId: "session-4",
			data: started,
		});
	...
	it("preserves transcript event objects when callers mutate task objects later", () => {
		...
		delivery.recordSessionEvent("session-6", {
			type: "task-started",
			sessionId: "session-6",
			data: task,
		});
		...
		expect(parseSseEvents(res)).toEqual([
			...
			},
			{
				type: "task-completed",
				sessionId: "session-6",
				data: { taskId: "task-2", title: "Mutable task", status: "completed" },
			},
			{
				type: "tasks-sync",
				sessionId: "session-6",
				data: {
					tasks: [
						{ taskId: "task-2", title: "Mutable task", status: "completed" },
					],
				},
			},
	...
	it("clears replay after Replay Grace Period", () => {
		...
		delivery.recordSessionEvent("session-5", {
			type: "claude-done",
			sessionId: "session-5",
		});
...
describe("eventDelivery singleton", () => {
	it("provides replay through the same Interface used by callers", () => {
		const sessionId = "singleton-session-1";
		eventDelivery.startSession(sessionId);
		eventDelivery.recordSessionEvent(sessionId, {
			type: "claude-message",
			sessionId,
			data: { type: "text", content: "test" },
		});

		expect(eventDelivery.hasReplay(sessionId)).toBe(true);

		const res = createSseResponse();
		eventDelivery.replayToSSE(sessionId, res);
		...
		expect(eventDelivery.hasReplay(sessionId)).toBe(false);
	});
});
